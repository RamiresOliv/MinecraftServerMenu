...
bitsadmin.exe /transfer "LICENSE" https://serverminecraftmenu.ramiresoliv.repl.co/LICENSE %mypath:~0,-1%\LICENSE > nul
if exist LICENSE (
...
msg * Porfavor fassa Download do Server.bat primeiro antes de atualizar!
goto menu
)
cls
echo um momento...
echo lembre-se isso pode demorar um pouco...
bitsadmin.exe /transfer "JobName" https://serverminecraftmenu.ramiresoliv.repl.co/version.txt %mypath:~0,-1%\VersionId.txt > nul
if errorlevel 1 ( 
call :colorEcho 04 "[FATAL ERROR] "
...
bitsadmin.exe /transfer "Instaler" https://serverminecraftmenu.ramiresoliv.repl.co/Downloads/Atualizador.bat %mypath:~0,-1%\Atualizador_TEMP_FILE.bat > nul
...
if %opcao% equ Limpa ( del "LICENSE" )
...
:Redirecionamento
if exist Atualizador_TEMP_FILE.bat () else ( del "Atualizador_TEMP_FILE.bat" )
call :colorEcho 06 "[WARN] "
echo : algum tipo de programa redirecionou voce para o menu novamente provavelmente foi a atualizacao ou outra coisa.
echo.
goto menu
